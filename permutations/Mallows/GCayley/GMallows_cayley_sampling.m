function [NewPop] = GMallows_cayley_sampling(NumbVar,model,Card,AuxPop,AuxFunVal,sampling_params)
% [NewPop] = GMallows_cayley_sampling(NumbVar,model,Card,AuxPop,AuxFunVal,sampling_params)
% GMallows_cayley_sampling:  Samples a population of individuals from an Generalized Mallows cayley model
%             
% INPUTS
...
for j=1:N,
    ...
    aux_x(randVal >= XProbs(i)') = 1; %if randVal < XProbs(i) then aux_x(i) = 0;
